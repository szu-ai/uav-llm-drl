...
# ---------------------------------------------------------------------
# Fixed-speed WITH governor baseline version.
# Raw speed command is constant (--fixed-speed); no PPO model is trained or
# loaded. The risk/corridor/yaw governor remains enabled, so this isolates
# the contribution of the governor from the learned PPO policy.
# ---------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fixed-speed + governor UAV baseline for e1 power-plant inspection with GUI viewer modes and full VSLAM trajectory logging.")
    ...
    p.add_argument("--fixed-speed", type=float, default=1.00,
                   help="Fixed raw speed v0 in m/s for the fixed-speed WITH governor baseline. This replaces the learned PPO policy output.")
    p.add_argument("--baseline-name", type=str, default="fixed_speed_governor",
    ...
    p.add_argument("--base-wind-mps", type=float, default=1.25,
                   help="Base wind speed in m/s for the fixed-speed/no-governor stress baseline. Increased from calm validation to expose drift risk.")
    p.add_argument("--wind-jitter", type=float, default=0.35,
...
class NPPDroneGymEnv(gym.Env):
    ...
    def __init__(self, args: argparse.Namespace, render_sim: bool = True):
        ...
        # Environment stress / lighting.  These match the main uav_llm.py interface,
        # but the fixed-speed/no-governor baseline uses a stronger default wind
        # so it is a meaningful stress baseline rather than a calm-flight replay.
        self.base_illumination_lux = float(max(50.0, getattr(args, "base_illumination_lux", 720.0)))
        self.illumination_jitter = float(max(0.0, getattr(args, "illumination_jitter", 35.0)))
        self.base_wind_mps = float(max(0.0, getattr(args, "base_wind_mps", 1.25)))
        self.wind_jitter = float(max(0.0, getattr(args, "wind_jitter", 0.35)))
        self.base_texture_count = int(max(1, getattr(args, "base_texture_count", 90)))
        ...
        # Fixed-speed WITH governor baseline.  There is no learned policy in this
        # file.  The fixed raw speed v0 is passed into the same governor used by
        # the proposed PPO method, so the experiment isolates the contribution
        # of learning from the safety/governor layer.
        self.fixed_speed_mps = float(np.clip(getattr(args, "fixed_speed", 1.0), self.osd_vmin, self.osd_vmax))
        self.baseline_name = str(getattr(args, "baseline_name", "fixed_speed_governor") or "fixed_speed_governor")
        print(f"[BASELINE] {self.baseline_name}: raw fixed speed v0={self.fixed_speed_mps:.3f} m/s; governor ON")
    ...
    def _lighting(self) -> None:
        ...
        # Stable daylight values following the main uav_llm.py style.
        # The proxy illumination value is logged in lux, while the USD light
        # intensities are scaled from that value to keep RGB output bright but
        # not over-exposed in headless Isaac runs.
        self._apply_lighting_from_domain()

    def _apply_lighting_from_domain(self) -> None:
        if self.stage is None:
            return
        try:
            lux = float(max(50.0, getattr(self, "illumination_lux", getattr(self, "base_illumination_lux", 720.0))))
            scale = float(np.clip(lux / 720.0, 0.55, 1.35))

            dome = UsdLux.DomeLight(self.stage.GetPrimAtPath("/World/Lights/SkyDome"))
            if dome:
                dome.GetIntensityAttr().Set(float(380.0 * scale))
                dome.GetColorAttr().Set(Gf.Vec3f(0.62, 0.72, 0.92))

            sun = UsdLux.DistantLight(self.stage.GetPrimAtPath("/World/Lights/MiddaySun"))
            if sun:
                sun.GetIntensityAttr().Set(float(3600.0 * scale))
            ...
            fill = UsdLux.SphereLight(self.stage.GetPrimAtPath("/World/Lights/CoolFill"))
            if fill:
                fill.GetIntensityAttr().Set(float(160.0 * scale))
            ...
            warm = UsdLux.SphereLight(self.stage.GetPrimAtPath("/World/Lights/WarmBounce"))
            if warm:
                warm.GetIntensityAttr().Set(float(70.0 * scale))
            ...
            print(f"[LIGHT] domain lighting update skipped: {exc}")
    ...
    def _vslam_risk(self, nav: Dict[str, float]) -> float:
        ...
        a lower feature-count saturation point for the power-plant route.
        """
    ...
    def _randomize_domain(self) -> None:
        ...
        if self.domain_randomization:
            lo_tex = max(1, self.base_texture_count - self.texture_count_jitter)
            hi_tex = max(lo_tex + 1, self.base_texture_count + self.texture_count_jitter + 1)
            self.texture_count = int(self.rng.integers(lo_tex, hi_tex))
            self.illumination_lux = float(max(50.0, self.rng.normal(self.base_illumination_lux, self.illumination_jitter)))
            self.wind_mps = float(max(0.0, self.rng.normal(self.base_wind_mps, self.wind_jitter)))
        else:
            self.texture_count = int(self.base_texture_count)
            self.illumination_lux = float(self.base_illumination_lux)
            self.wind_mps = float(self.base_wind_mps)
        self._apply_lighting_from_domain()
        print(f"[DOMAIN] texture={self.texture_count:d} illumination={self.illumination_lux:.1f} lux wind={self.wind_mps:.2f} m/s")

    def step(self, action: np.ndarray):
        a = np.clip(np.asarray(action, np.float32).reshape(-1), self.action_space.low, self.action_space.high)
        self.step_count += 1

        # Fixed-speed WITH governor baseline:
        # ignore the action/PPO policy and use a constant raw speed v0.
        # The VSLAM/corridor/yaw governor remains enabled.
        # This means risk-based speed reduction, safety gate, and
        # fallback/abort decision from the proposed governor are active.
        policy_speed = float(np.clip(self.fixed_speed_mps, self.osd_vmin, self.osd_vmax))
        nav = self._nav_metrics()
        self._nav_cache = dict(nav)
        z_t = self._vslam_risk(nav)
        d_t, phi_t = self._route_errors()

        # Fixed-speed + governor baseline:
        # the policy side is fixed, but the same safety governor used by the
        # proposed method remains active.  This lets the paper compare
        # "Fixed speed / No Gov." against "Fixed speed / Gov." cleanly.
    ...
    def _finalize_episode(self, info: Dict[str, Any], terminated: bool, truncated: bool) -> None:
        ...
        row = {
            ...
            "fixed_speed_mps": float(self.fixed_speed_mps),
    ...
    def _write_summary(self) -> None:
        ...
        summary = {
            "run_id": self.run_id,
            "baseline": str(getattr(self, "baseline_name", "fixed_speed_governor")),
            "fixed_speed_mps": float(getattr(self, "fixed_speed_mps", 1.0)),
            "episodes": n,
            "route_loops": int(getattr(self, "effective_route_loops", 1)),
            "requested_route_repeat_count": int(self.route_repeat_count),
            "repeat_full_route": int(bool(getattr(self, "repeat_full_route", False))),
    ...
    def _trajectory_dicts(self) -> List[Dict[str, float]]:
        ...
        for i, rec in enumerate(getattr(self, "trajectory_records", []) or []):
            ...
            rows.append({
                ...
                "sim_env_id": "e1",
                "policy_name": str(getattr(self, "baseline_name", "fixed_speed_governor")),
...
def _fixed_action_for_logging(env: NPPDroneGymEnv) -> np.ndarray:
    """Return a dummy action; NPPDroneGymEnv.step() ignores it in this baseline."""
    return np.array([0.0], dtype=np.float32)


def run_fixed_speed_governor(env: NPPDroneGymEnv, args: argparse.Namespace, label: str = "EVAL") -> None:
    """Evaluate fixed-speed WITH governor baseline without loading or training PPO.

    The raw speed is constant, v_t = --fixed-speed.  The environment still
    computes VSLAM risk, route-error, collision, energy, and drift metrics,
    and the proposed risk/corridor/yaw governor remains active.  This baseline
    isolates the effect of the safety governor when the speed policy is fixed.
    """
    print(
        f"[{label}] fixed-speed WITH governor baseline: episodes={int(args.eval_episodes)}, "
        f"v0={env.fixed_speed_mps:.3f} m/s, output={env.output_root}"
    )
    for ep in range(int(args.eval_episodes)):
        obs, _ = env.reset(seed=args.seed + ep)
        done = trunc = False
        while not (done or trunc):
            obs, _r, done, trunc, _info = env.step(_fixed_action_for_logging(env))
...
def train(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    # A fixed-speed baseline has no trainable policy.  Keep --mode train as a
    # convenience alias so old scripts do not crash; it simply runs evaluation.
    print("[BASELINE] --mode train requested, but fixed_speed_governor has no PPO training. Running fixed-speed governor baseline evaluation instead.")
    run_fixed_speed_governor(env, args, label="FIXED_GOV_TRAIN_ALIAS")


def evaluate(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    run_fixed_speed_governor(env, args, label="FIXED_GOV_EVAL")
...
def demo(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    ...
    try:
        run_fixed_speed_governor(env, args, label="FIXED_GOV_DEMO")
    finally:
...
def main() -> None:
    ...
    if SimulationApp is None:
        raise RuntimeError(
            ...
            "  cd ~/IsaacLab && ./isaaclab.sh -p source/standalone/uav_llm/uav_fix.py --mode eval --headless"
        )
