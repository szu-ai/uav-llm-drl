...
# ---------------------------------------------------------------------
# Fixed-speed + governor baseline version.
# Raw speed command is constant (--fixed-speed); no PPO model is trained or
# loaded. The original risk/corridor/yaw governor and logging pipeline remain.
# ---------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fixed-speed", type=float, default=1.00,
                   help="Fixed raw speed v0 in m/s for the fixed-speed + governor baseline. This replaces the learned PPO policy output.")
...
class NPPDroneGymEnv(gym.Env):
    ...
    def __init__(self, args: argparse.Namespace, render_sim: bool = True):
        ...
        # Fixed-speed + governor baseline.  There is no learned policy in this
    ...
    def step(self, action: np.ndarray):
        ...
        # The same risk/corridor/yaw governor then computes the executed command.
...
def run_fixed_speed_governor(env: NPPDroneGymEnv, args: argparse.Namespace, label: str = "EVAL") -> None:
    """Evaluate fixed-speed + governor baseline without loading or training PPO.

    The raw speed is constant, v_t = --fixed-speed.  The environment still
    applies the same VSLAM-risk, corridor, yaw, rate-limit, and fallback
    governor as the proposed learned controller.  This isolates the effect of
    the learned PPO speed policy from the safety/governor layer.
    """
    print(
        f"[{label}] fixed-speed + governor baseline: episodes={int(args.eval_episodes)}, "
...
def train(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    ...
    print("[BASELINE] --mode train requested, but fixed_speed_governor has no PPO training. Running baseline evaluation instead.")
    run_fixed_speed_governor(env, args, label="FIXED_TRAIN_ALIAS")


def evaluate(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    run_fixed_speed_governor(env, args, label="FIXED_EVAL")
...
def demo(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    ...
    try:
        run_fixed_speed_governor(env, args, label="FIXED_DEMO")
    finally:
