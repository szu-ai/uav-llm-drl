...
# ---------------------------------------------------------------------
# PID speed scheduler + governor baseline for the e2 industrial transfer
# domain. Raw speed is produced by a classical PID feedback controller on
# the VSLAM risk score: e_t = z_t - z_ref, u_t = Kp e_t + Ki int(e) + Kd de/dt,
# v_raw = base_speed - u_t. No PPO model is trained or loaded; the same
# risk/corridor/yaw governor and full e2 logging/artifact pipeline remain.
# ---------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="PID speed scheduler + governor baseline for e2 industrial-domain UAV VSLAM evaluation.")

    # Runtime
    p.add_argument("--headless", action="store_true")
    p.add_argument("--mode", type=str, default="eval", choices=["eval", "demo"], help="Evaluation-only PID baseline. eval runs PID + governor; demo runs one visible PID rollout.")
    ...
    p.add_argument("--model-path", type=str, default="", help="Ignored in PID baseline; accepted for command compatibility.")
    ...
    p.add_argument("--world-size", type=float, default=80.0,
                   help="Half-extent of the site boundary. Enlarged so the expanded multi-district plant (Ã¢ÂÂ2x footprint) fits inside the fence. Observations are normalized by max_ray_range, not world-size, so transfer scaling is unaffected.")
    ...
    p.add_argument("--pid-base-speed", type=float, default=0.0,
                   help="Nominal raw speed before PID correction. Use 0.0 to default to --speed-max.")
    p.add_argument("--pid-z-ref", type=float, default=0.25,
                   help="Reference VSLAM risk. PID slows the UAV when z_t exceeds this value.")
    p.add_argument("--pid-kp", type=float, default=0.75,
                   help="PID proportional gain for VSLAM-risk error e_t=z_t-z_ref.")
    p.add_argument("--pid-ki", type=float, default=0.03,
                   help="PID integral gain for accumulated VSLAM-risk error.")
    p.add_argument("--pid-kd", type=float, default=0.18,
                   help="PID derivative gain for VSLAM-risk error change.")
    p.add_argument("--pid-integral-limit", type=float, default=2.0,
                   help="Anti-windup clamp for the PID integral state.")
    p.add_argument("--baseline-name", type=str, default="pid_governor_e2",
                   help="Name written into logs for this baseline run.")
    ...
    # Environment stress / lighting (matched with e1 PID/proposed runs)
...
class IndustrialE2EvalEnv(gym.Env):
    ...
    def __init__(self, args: argparse.Namespace, render_sim: bool = True):
        ...
        # PID speed scheduler + governor baseline. There is no learned policy in
        # this file. The raw speed is computed from VSLAM-risk feedback:
        #     e_t = z_t - z_ref
        #     u_t = Kp*e_t + Ki*integral(e_t) + Kd*d(e_t)/dt
        #     v_raw = base_speed - u_t
        # and then passed to the same governor used by the proposed controller.
        self.pid_base_speed = float(getattr(args, "pid_base_speed", 0.0))
        if self.pid_base_speed <= 0.0:
            self.pid_base_speed = float(self.osd_vmax)
        self.pid_base_speed = float(np.clip(self.pid_base_speed, self.osd_vmin, self.osd_vmax))
        self.pid_z_ref = float(np.clip(getattr(args, "pid_z_ref", 0.25), 0.0, 1.0))
        self.pid_kp = float(max(0.0, getattr(args, "pid_kp", 0.75)))
        self.pid_ki = float(max(0.0, getattr(args, "pid_ki", 0.03)))
        self.pid_kd = float(max(0.0, getattr(args, "pid_kd", 0.18)))
        self.pid_integral_limit = float(max(0.05, getattr(args, "pid_integral_limit", 2.0)))
        self.pid_integral = 0.0
        self.pid_prev_error = 0.0
        self.pid_last_control = 0.0
        self.baseline_name = str(getattr(args, "baseline_name", "pid_governor_e2") or "pid_governor_e2")
        # Keep this field for backward-compatible metric parsing; it represents
        # the nominal base speed used by the PID scheduler, not a constant command.
        self.fixed_speed_mps = float(self.pid_base_speed)
        print(
            f"[BASELINE] {self.baseline_name}: v_raw=base-(Kp*e+Ki*int+Kd*de), "
            f"base={self.pid_base_speed:.3f} m/s, z_ref={self.pid_z_ref:.3f}, "
            f"Kp={self.pid_kp:.3f}, Ki={self.pid_ki:.3f}, Kd={self.pid_kd:.3f}; governor ON"
        )
        ...
        self.illumination_jitter = float(max(0.0, getattr(args, "illumination_jitter", 35.0)))
        self.base_wind_mps = float(np.clip(getattr(args, "base_wind_mps", 0.45), 0.0, 2.2))
        self.wind_jitter = float(max(0.0, getattr(args, "wind_jitter", 0.10)))
        ...
        print("[ENV_INIT] e2 PID industrial transfer scene ready. reset() deferred.")
    ...
    def _ground(self) -> None:
        ...
        # Main paved areas covering the enlarged (Ã¢ÂÂ2x) footprint: north plant
    ...
    def _reset_slam(self) -> None:
        ...
        # Raw/aligned cuVSLAM diagnostics; proxy/gt mode leaves these harmless.
    ...
    def _make_low_feature_zones(self) -> List[dict]:
        """Create route-aligned degraded visual-feature corridors for e2."""
        zones: List[dict] = []
        ...
        if pts.ndim != 2 or pts.shape[0] < 8:
            return zones
        # Spread degraded corridors across the e2 industrial sweep. Use relative
        # indices so it works for both 4x4 and expanded adaptive e2 grids.
        n = int(len(pts))
        candidate_ids = sorted(set([max(0, int(0.22 * (n - 1))), max(0, int(0.50 * (n - 1))), max(0, int(0.78 * (n - 1)))]))
        half = float(max(2.0, getattr(self, "low_feature_zone_width", 8.0)))
        for zid, idx in enumerate(candidate_ids):
            if idx >= len(pts):
                continue
            p = pts[idx]
            q = pts[min(idx + 1, len(pts) - 1)] if idx + 1 < len(pts) else pts[max(idx - 1, 0)]
            center = 0.5 * (p[:2] + q[:2])
            mn = np.array([center[0] - half, center[1] - 0.72 * half, -5.0], np.float32)
            mx = np.array([center[0] + half, center[1] + 0.72 * half, 80.0], np.float32)
    ...
    def _align_cuvslam_odom(self, odom: Dict[str, Any], true_pos: np.ndarray, true_vel: np.ndarray, true_yaw: float) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        self.cuvslam_raw_yaw = float(raw_yaw)
        ...
        last_true = getattr(self, "cuvslam_last_pair_true", np.array([np.nan, np.nan, np.nan], np.float32))
        if (not np.all(np.isfinite(last_true))) or float(np.linalg.norm(np.asarray(true_pos, np.float32) - last_true)) > 0.08:
        ...
        if bool(getattr(self, "cuvslam_align_ready", False)):
            rel_raw = raw_pos - np.asarray(self.cuvslam_raw_origin, np.float32)
            feat = np.concatenate([rel_raw.astype(np.float32), np.ones((1,), np.float32)], axis=0)
            ...
            aligned_vel = (raw_vel @ Wlin).astype(np.float32)
            aligned_yaw = float(true_yaw + self._wrap(raw_yaw - self.cuvslam_raw_yaw0) * 0.35)
            return aligned_pos, aligned_vel, aligned_yaw
        return np.asarray(true_pos, np.float32).copy(), np.asarray(true_vel, np.float32).copy(), float(true_yaw)
    ...
    def _visible_features(self) -> int:
        ...
        # The base texture count is a slow domain variable; low-feature corridors
        # locally reduce features to model motion-blur/weak-texture transfer risk.
        texture_scale = float(np.clip(getattr(self, "texture_count", 90) / 90.0, 0.35, 1.65))
        raw *= texture_scale * self._low_feature_multiplier_at()
    ...
    def _update_slam(self) -> None:
        ...
        feature_health = float(np.clip(fc / 45.0, 0.0, 1.0))
        low_feature = float(1.0 - feature_health)
        motion_blur_risk = float(np.clip(blur * low_feature, 0.0, 1.0))
        if self._in_low_feature_corridor():
            motion_blur_risk = max(motion_blur_risk, 0.65 * blur)
        drift_gain = 1.0 + self.motion_blur_drift_gain * motion_blur_risk
        loss_gain = 1.0 + self.motion_blur_loss_gain * motion_blur_risk
        scale = 0.55 + 0.70 * (1.0 - stability)
        self.slam_drift = (
            0.997 * self.slam_drift
            + self.rng.normal(0, self.slam_drift_pos_per_sec * self.dt * drift_gain * (0.35 + 0.85 * (1 - stability)), 3).astype(np.float32)
        )
        self.slam_drift[2] *= 0.35
        loss_prob = self.slam_tracking_loss_prob * loss_gain * (0.55 + 1.40 * (1 - stability))
        if self.rng.random() < loss_prob:
            self.slam_quality = float(self.rng.uniform(0.08, 0.36))
            self.slam_pos = (self.slam_pos + 0.04 * self.rng.normal(0, self.slam_pos_noise_std * scale * drift_gain, 3)).astype(np.float32)
            self.slam_vel = (0.9 * self.slam_vel).astype(np.float32)
        else:
            recover = self.slam_quality_recover_rate * (0.35 + 0.65 * stability) * (1.0 - 0.45 * motion_blur_risk)
            self.slam_quality = float(min(1.0, self.slam_quality + recover))
            self.slam_pos = (true_pos + self.slam_drift + self.rng.normal(0, self.slam_pos_noise_std * scale * drift_gain, 3)).astype(np.float32)
            self.slam_vel = (true_vel + self.rng.normal(0, self.slam_vel_noise_std * scale * drift_gain, 3)).astype(np.float32)
        self.slam_yaw = true_yaw + float(self.rng.normal(0, self.slam_yaw_noise_std * scale * drift_gain))
    ...
    def _randomize_domain(self) -> None:
        ...
        if self.domain_randomization:
            tex = int(round(float(self.base_texture_count) + float(self.rng.normal(0.0, self.texture_count_jitter))))
            self.texture_count = int(np.clip(tex, 20, 180))
    ...
    def step(self, action: np.ndarray):
        ...
        # PID speed scheduler + governor baseline:
        # ignore the action/PPO policy and compute a classical PID raw speed from
        # the current VSLAM risk score. The PID slows the UAV when z_t exceeds
        # z_ref, then the same risk/corridor/yaw governor checks the command.
        nav = self._nav_metrics()
        self._nav_cache = dict(nav)
        z_t = self._vslam_risk(nav)
        d_t, phi_t = self._route_errors()
        err = float(z_t - self.pid_z_ref)
        self.pid_integral = float(np.clip(self.pid_integral + err * self.dt, -self.pid_integral_limit, self.pid_integral_limit))
        derr = float((err - self.pid_prev_error) / max(self.dt, 1e-6))
        self.pid_prev_error = float(err)
        u_pid = float(self.pid_kp * err + self.pid_ki * self.pid_integral + self.pid_kd * derr)
        self.pid_last_control = float(u_pid)
        policy_speed = float(np.clip(self.pid_base_speed - u_pid, self.osd_vmin, self.osd_vmax))
        ...
        if dist_xy > 2.4:
            # Do not force high speed in weak-feature/risky regions. A hard
            # transit floor would hide whether PID can slow down where VSLAM
            # needs slower motion.
        ...
        # Real localization failure condition. This prevents every baseline from
        # receiving Ps=1.0 when it charges through low-feature corridors after
        # the proxy SLAM estimate has diverged.
    ...
    def _record_step(self, commanded: float, reward: float, info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self._in_low_feature_corridor():
            m["low_feature_steps"] += 1
        if float(loc) > float(getattr(self, "ate_fail_threshold", 1.25)):
        ...
        self.trajectory_records.append([
            ...
            float(getattr(self, "pid_base_speed", self.osd_vmax)),
            float(getattr(self, "last_policy_speed_mps", 0.0)),
            float(getattr(self, "pid_last_control", 0.0)),
            float(getattr(self, "pid_prev_error", 0.0)),
    ...
    def _finalize_episode(self, info: Dict[str, Any], terminated: bool, truncated: bool) -> None:
        ...
        success = bool(route_complete and not collision and not localization_failure)
        ...
        row = {
            ...
            "run_id": self.run_id,
            "env": "e2_industrial",
            "baseline_name": str(getattr(self, "baseline_name", "pid_governor_e2")),
            "pid_base_speed_mps": float(getattr(self, "pid_base_speed", getattr(self, "osd_vmax", 1.0))),
            "pid_z_ref": float(getattr(self, "pid_z_ref", 0.25)),
            "pid_kp": float(getattr(self, "pid_kp", 0.0)),
            "pid_ki": float(getattr(self, "pid_ki", 0.0)),
            "pid_kd": float(getattr(self, "pid_kd", 0.0)),
            "base_wind_mps": float(getattr(self, "base_wind_mps", 0.45)),
            "base_illumination_lux": float(getattr(self, "base_illumination_lux", 720.0)),
            "base_texture_count": int(getattr(self, "base_texture_count", 90)),
            "baseline_name": str(getattr(self, "baseline_name", "pid_governor_e2")),
            "pid_base_speed_mps": float(getattr(self, "pid_base_speed", getattr(self, "osd_vmax", 1.0))),
            "pid_z_ref": float(getattr(self, "pid_z_ref", 0.25)),
            "pid_kp": float(getattr(self, "pid_kp", 0.0)),
            ...
            "low_feature_steps_pct": float(m.get("low_feature_steps", 0)) * per100,
    ...
    def _write_summary(self) -> None:
        ...
        summary = {
            ...
            "low_feature_steps_pct": mean("low_feature_steps_pct"),
    ...
    def _trajectory_dicts(self) -> List[Dict[str, float]]:
        ...
        for i, rec in enumerate(getattr(self, "trajectory_records", []) or []):
            if isinstance(rec, dict):
                ...
                d.setdefault("policy_name", str(d.get("policy_name", getattr(self, "baseline_name", "pid_governor_e2"))))
                d.setdefault("sim_env_id", str(d.get("sim_env_id", "e2")))
                d.setdefault("low_feature_multiplier", float(d.get("low_feature_multiplier", 1.0)))
                d.setdefault("low_feature_zone", float(d.get("low_feature_zone", 0.0)))
                d.setdefault("localization_failure_streak", float(d.get("localization_failure_streak", 0.0)))
                d.setdefault("pid_policy_speed_mps", float(d.get("pid_policy_speed_mps", d.get("speed_mps", 0.0))))
                rows.append(d)
                continue
            ...
            pid_base_speed = val(23, float(getattr(self, "pid_base_speed", getattr(self, "osd_vmax", 1.0))))
            pid_policy_speed = val(24, speed)
            pid_control = val(25, 0.0)
            pid_error = val(26, 0.0)
            cuvslam_raw_x = val(27, sx)
            cuvslam_raw_y = val(28, sy)
            cuvslam_raw_z = val(29, sz)
            cuvslam_align_ready = val(30, 0.0)
            cuvslam_align_rmse = val(31, -1.0)
            rows.append({
                ...
                "sim_env_id": "e2",
                "policy_name": str(getattr(self, "baseline_name", "pid_governor_e2")),
                ...
                "pid_base_speed_mps": pid_base_speed,
                "pid_policy_speed_mps": pid_policy_speed,
...
def _pid_dummy_action(env: IndustrialE2EvalEnv) -> np.ndarray:
    """Return a dummy action; IndustrialE2EvalEnv.step() ignores it in this PID baseline."""
    return np.array([0.0], dtype=np.float32)


def run_pid_speed_governor(env: IndustrialE2EvalEnv, args: argparse.Namespace, label: str = "EVAL") -> None:
    """Evaluate PID speed scheduler + governor baseline without PPO.

    The raw speed is generated by a classical PID feedback controller over the
    VSLAM-risk error e_t=z_t-z_ref. The same VSLAM-risk, corridor, yaw,
    rate-limit, and fallback governor is then applied in the e2 industrial
    transfer domain.
    """
    print(
        f"[{label}] PID speed scheduler + governor e2 baseline: episodes={int(args.eval_episodes)}, "
        f"base={env.pid_base_speed:.3f}, z_ref={env.pid_z_ref:.3f}, "
        f"Kp={env.pid_kp:.3f}, Ki={env.pid_ki:.3f}, Kd={env.pid_kd:.3f}, output={env.output_root}"
    )
    for ep in range(int(args.eval_episodes)):
        obs, _ = env.reset(seed=args.seed + ep)
        done = trunc = False
        while not (done or trunc):
            obs, _r, done, trunc, _info = env.step(_pid_dummy_action(env))
    print(f"[{label}] metrics -> {env.summary_json}")
    print(f"[{label}] paper row -> {env.result_table_csv}")


def evaluate(env: IndustrialE2EvalEnv, args: argparse.Namespace) -> None:
    run_pid_speed_governor(env, args, label="PID_E2_EVAL")


def demo(env: IndustrialE2EvalEnv, args: argparse.Namespace) -> None:
    old_eps = int(args.eval_episodes)
    args.eval_episodes = 1
    try:
        run_pid_speed_governor(env, args, label="PID_E2_DEMO")
    finally:
        args.eval_episodes = old_eps


def main() -> None:
    args = parse_args()
    # Evaluation-only PID baseline; keep demo/eval compatibility.
