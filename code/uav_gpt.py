...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="structured",
        choices=["structured", "llama_hf", "openai_chatgpt", "chatgpt"],
        help="structured parser, Hugging Face LLaMA, or OpenAI ChatGPT/API mission encoder. chatgpt is an alias for openai_chatgpt.",
    )
    ...
    p.add_argument("--openai-model", type=str, default="gpt-4o-mini", help="OpenAI model for --mission-encoder openai_chatgpt.")
    p.add_argument("--openai-api-key", type=str, default=os.environ.get("OPENAI_API_KEY", ""), help="OpenAI API key. Prefer setting OPENAI_API_KEY in the shell.")
    p.add_argument("--openai-base-url", type=str, default=os.environ.get("OPENAI_BASE_URL", ""), help="Optional compatible base URL/proxy endpoint.")
    p.add_argument("--openai-timeout", type=float, default=30.0, help="Timeout in seconds for the ChatGPT mission encoder call.")
    ...
    if getattr(args, "mission_encoder", "") == "chatgpt":
        args.mission_encoder = "openai_chatgpt"
    return args
...
class MissionEncoder:
    ...
    def __init__(
        ...
        self.openai_model = str(openai_model or "gpt-4o-mini")
        self.openai_api_key = str(openai_api_key or os.environ.get("OPENAI_API_KEY", ""))
        self.openai_base_url = str(openai_base_url or os.environ.get("OPENAI_BASE_URL", ""))
        self.openai_timeout = float(max(1.0, openai_timeout))
        self.tokenizer = None
        self.model = None
        self.torch = None
        ...
        elif self.backend == "openai_chatgpt":
            self._try_load_openai()
    ...
    def _try_load_openai(self) -> None:
        """Initialize OpenAI ChatGPT/API mission encoder.

        This encoder is called only for the compact mission string, not inside
        the real-time UAV control loop. If OpenAI is unavailable, the code falls
        back to the deterministic structured parser so PPO can still run.
        """
        try:
            from openai import OpenAI
            kwargs: Dict[str, Any] = {"api_key": self.openai_api_key, "timeout": self.openai_timeout}
            ...
            self.loaded = True
            print(f"[ChatGPT] OpenAI mission encoder ready: {self.openai_model}")
        except Exception as exc:
            self.loaded = False
            self.openai_client = None
            print(f"[ChatGPT] Falling back to structured parser. Error: {exc}")

    def _mission_schema(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "E": {"type": "string", "enum": self.ENV_TOKENS},
                "G": {"type": "string", "enum": self.GOAL_TOKENS},
                "S": {"type": "string", "enum": self.SAFETY_TOKENS},
                "O": {"type": "string", "enum": self.OP_TOKENS},
                "lambda": {"type": "number", "minimum": 0.0, "maximum": 1.0},
            },
            "required": ["E", "G", "S", "O", "lambda"],
            "additionalProperties": False,
        }

    def _mission_prompt(self, text: str) -> str:
        return (
            ...
            "Choose lambda as a risk preference in [0,1]. "
            "Use higher lambda for low-texture, low-light, repetitive, narrow, tracking-loss, cautious, or fail-safe missions.\n"
            'Return only JSON like {"E":"low-texture","G":"coverage","S":"feature-slow","O":"slow-smooth","lambda":0.78}.\n'
            f"Mission: {text}"
        )

    def _validate_parsed(self, parsed: Dict[str, Any], base: Dict[str, Any]) -> Dict[str, Any]:
        checked = {
    ...
    def _parse_with_openai(self, text: str) -> Dict[str, Any]:
        ...
        if not self.loaded or self.openai_client is None:
            return base
        prompt = self._mission_prompt(text)
        try:
            # New OpenAI Responses API with Structured Outputs.
            resp = self.openai_client.responses.create(
                model=self.openai_model,
                input=[
                    {
                        "role": "system",
                        "content": "You are a deterministic UAV mission encoder. Output only the requested JSON object.",
                    },
                    {"role": "user", "content": prompt},
                ],
                max_output_tokens=max(48, self.max_new_tokens),
                text={
                    "format": {
                        "type": "json_schema",
                        "name": "uav_mission_encoding",
                        "schema": self._mission_schema(),
                        "strict": True,
                    }
                },
            )
            raw = getattr(resp, "output_text", "") or str(resp)
            parsed = json.loads(raw)
            return self._validate_parsed(parsed, base)
            ...
            try:
                ...
                resp = self.openai_client.chat.completions.create(
                    model=self.openai_model,
                    messages=[
                        {"role": "system", "content": "You are a deterministic UAV mission encoder. Output only JSON."},
                        {"role": "user", "content": prompt},
                    ],
                    max_tokens=max(48, self.max_new_tokens),
                    temperature=0,
                    response_format={"type": "json_object"},
                )
                raw = resp.choices[0].message.content or "{}"
                parsed = self._first_json(raw) or json.loads(raw)
                return self._validate_parsed(parsed, base)
            except Exception as exc2:
                print(f"[ChatGPT] parse failed ({type(exc1).__name__}/{type(exc2).__name__}); using structured parser.")
                return base
    ...
    def encode(self, text: str) -> Tuple[np.ndarray, float, Dict[str, Any]]:
        if self.backend == "llama_hf":
            parsed = self._parse_with_llama(text)
        elif self.backend == "openai_chatgpt":
            parsed = self._parse_with_openai(text)
        else:
