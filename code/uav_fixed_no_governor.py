...
# ---------------------------------------------------------------------
# Fixed-speed WITHOUT governor baseline version.
# Raw speed command is constant (--fixed-speed); no PPO model is trained or
# loaded. The risk/corridor/yaw governor is disabled, but the logging pipeline remains.
# ---------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fixed-speed", type=float, default=1.00,
                   help="Fixed raw speed v0 in m/s for the fixed-speed WITHOUT governor baseline. This replaces the learned PPO policy output.")
    p.add_argument("--baseline-name", type=str, default="fixed_speed_no_governor",
...
class NPPDroneGymEnv(gym.Env):
    ...
    def __init__(self, args: argparse.Namespace, render_sim: bool = True):
        ...
        # Fixed-speed WITHOUT governor baseline.  There is no learned policy in this
        ...
        self.baseline_name = str(getattr(args, "baseline_name", "fixed_speed_no_governor") or "fixed_speed_no_governor")
        print(f"[BASELINE] {self.baseline_name}: raw fixed speed v0={self.fixed_speed_mps:.3f} m/s; governor OFF")
    ...
    def step(self, action: np.ndarray):
        ...
        # Fixed-speed WITHOUT governor baseline:
        # ignore the action/PPO policy and use a constant raw speed v0.
        # The VSLAM/corridor/yaw governor is intentionally disabled.
        # This means no risk-based speed reduction, no safety gate, and no
        # fallback/abort decision from the proposed governor.  We still compute
        # z_t, d_t, and phi_t for logging only.
        ...
        commanded = float(policy_speed)
        gate, abort, reason = 1, 0, "no_governor"
        self.last_policy_speed = float(policy_speed)
        self.last_commanded_speed = float(commanded)
        self.last_governor_override = 0
    ...
    def _write_summary(self) -> None:
        ...
        summary = {
            "run_id": self.run_id,
            "baseline": str(getattr(self, "baseline_name", "fixed_speed_no_governor")),
    ...
    def _trajectory_dicts(self) -> List[Dict[str, float]]:
        ...
        for i, rec in enumerate(getattr(self, "trajectory_records", []) or []):
            ...
            rows.append({
                ...
                "sim_env_id": "e1",
                "policy_name": str(getattr(self, "baseline_name", "fixed_speed_no_governor")),
...
def run_fixed_speed_no_governor(env: NPPDroneGymEnv, args: argparse.Namespace, label: str = "EVAL") -> None:
    """Evaluate fixed-speed WITHOUT governor baseline without loading or training PPO.
    ...
    but the proposed risk/corridor/yaw governor is bypassed.  This baseline
    shows what happens with fixed-speed flight and no stack-level governor.
    """
    print(
        f"[{label}] fixed-speed WITHOUT governor baseline: episodes={int(args.eval_episodes)}, "
...
def train(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    ...
    print("[BASELINE] --mode train requested, but fixed_speed_no_governor has no PPO training. Running no-governor baseline evaluation instead.")
    run_fixed_speed_no_governor(env, args, label="FIXED_TRAIN_ALIAS")


def evaluate(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    run_fixed_speed_no_governor(env, args, label="FIXED_EVAL")
...
def demo(env: NPPDroneGymEnv, args: argparse.Namespace) -> None:
    ...
    try:
        run_fixed_speed_no_governor(env, args, label="FIXED_DEMO")
    finally:
