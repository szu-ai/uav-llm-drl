...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Standalone e2 fixed-speed WITH governor baseline for industrial-domain UAV inspection.")

    # Runtime
    p.add_argument("--headless", action="store_true")
    p.add_argument("--mode", type=str, default="eval", choices=["eval", "demo"], help="Evaluation-only fixed-speed + governor baseline. No PPO model is loaded.")
    ...
    p.add_argument("--fixed-speed", type=float, default=1.35,
                   help="Fixed raw speed in m/s for the e2 fixed-speed WITH governor baseline. PPO is not loaded.")
    p.add_argument("--baseline-name", type=str, default="e2_fixed_speed_governor",
    ...
    p.add_argument("--base-illumination-lux", type=float, default=720.0,
                   help="Base scene illumination used by the proxy/VSLAM scene model.")
    ...
    p.add_argument("--base-wind-mps", type=float, default=1.25,
                   help="Base wind speed in m/s for e2 fixed-speed stress validation.")
...
class IndustrialE2EvalEnv(gym.Env):
    ...
    def __init__(self, args: argparse.Namespace, render_sim: bool = True):
        ...
        self.fixed_speed_mps = float(np.clip(getattr(args, "fixed_speed", self.osd_vmax), self.osd_vmin, self.osd_vmax))
        self.baseline_name = str(getattr(args, "baseline_name", "e2_fixed_speed_governor") or "e2_fixed_speed_governor")
        print(f"[BASELINE] {self.baseline_name}: fixed raw speed={self.fixed_speed_mps:.3f} m/s; governor ON; PPO OFF")
        ...
        self.texture_count = int(getattr(args, "base_texture_count", 90))
        self.illumination_lux = float(getattr(args, "base_illumination_lux", 720.0))
        self.wind_mps = float(getattr(args, "base_wind_mps", 1.25))
    ...
    def _apply_lighting_from_domain(self) -> None:
        """Scale scene lights from the current illumination_lux value.

        This keeps the physical scene lighting consistent with the logged
        VSLAM/domain illumination variable used by the observation model.
        """
        try:
            if self.stage is None:
                return
            lux = float(np.clip(getattr(self, "illumination_lux", 720.0), 120.0, 1400.0))
            scale = lux / 720.0
            specs = [
                ("/World/Lights/SkyDome", UsdLux.DomeLight, 380.0),
                ("/World/Lights/MiddaySun", UsdLux.DistantLight, 3600.0),
                ("/World/Lights/CoolFill", UsdLux.SphereLight, 160.0),
                ("/World/Lights/WarmBounce", UsdLux.SphereLight, 70.0),
            ]
            for path, cls, base in specs:
                prim = self.stage.GetPrimAtPath(path)
                if prim and prim.IsValid():
                    cls(prim).GetIntensityAttr().Set(float(base * scale))
        except Exception as exc:
            if not getattr(self, "_light_warned", False):
                print(f"[LIGHTING] domain light update skipped: {exc}")
    ...
    def _randomize_domain(self) -> None:
        ...
        base_tex = int(getattr(self.args, "base_texture_count", 90))
        tex_jit = int(max(0, getattr(self.args, "texture_count_jitter", 25)))
        base_lux = float(getattr(self.args, "base_illumination_lux", 720.0))
        lux_jit = float(max(0.0, getattr(self.args, "illumination_jitter", 35.0)))
        base_wind = float(getattr(self.args, "base_wind_mps", 1.25))
        wind_jit = float(max(0.0, getattr(self.args, "wind_jitter", 0.35)))
        if self.domain_randomization:
            self.texture_count = int(np.clip(base_tex + int(self.rng.integers(-tex_jit, tex_jit + 1)), 5, 400))
            self.illumination_lux = float(max(50.0, base_lux + self.rng.uniform(-lux_jit, lux_jit)))
            self.wind_mps = float(max(0.0, base_wind + self.rng.uniform(-wind_jit, wind_jit)))
        else:
            self.texture_count = int(np.clip(base_tex, 5, 400))
            self.illumination_lux = float(max(50.0, base_lux))
            self.wind_mps = float(max(0.0, base_wind))
        self._apply_lighting_from_domain()
    ...
    def step(self, action: np.ndarray):
        ...
        # The action is ignored; PPO is not loaded.  The fixed raw speed v0 is
        # passed through the same VSLAM/corridor/yaw governor used by the main
        # method, so this baseline isolates the safety governor from learning.
        policy_speed = float(np.clip(getattr(self, "fixed_speed_mps", self.osd_vmax), self.osd_vmin, self.osd_vmax))
        ...
        self.last_governor_override = int(1 if abs(float(commanded) - float(policy_speed)) > 1e-6 else 0)
...
def evaluate(env: IndustrialE2EvalEnv, args: argparse.Namespace) -> None:
    """Run e2 fixed-speed + governor evaluation.

    This baseline intentionally does not load PPO. The environment ignores the
    action, uses --fixed-speed as the raw speed, and then applies the governor.
    """
    print(
        f"[E2_FIXED_GOV_EVAL] baseline={getattr(args, 'baseline_name', 'e2_fixed_speed_governor')} "
        f"fixed_speed={float(getattr(args, 'fixed_speed', 1.35)):.3f} m/s; PPO OFF; governor ON"
    )
    ...
    print(f"[E2_FIXED_GOV_EVAL] metrics -> {env.summary_json}")
    print(f"[E2_FIXED_GOV_EVAL] paper row -> {env.result_table_csv}")
...
def demo(env: IndustrialE2EvalEnv, args: argparse.Namespace) -> None:
    ...
    while not (done or trunc):
        obs, _r, done, trunc, _info = env.step(np.array([0.0], np.float32))
    print("[E2_FIXED_DEMO] rollout complete")
    print(f"[E2_FIXED_DEMO] metrics -> {env.summary_json}")
