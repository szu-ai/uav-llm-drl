...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Standalone e2 fixed-speed WITHOUT governor baseline for industrial-domain UAV inspection.")

    # Runtime
    p.add_argument("--headless", action="store_true")
    p.add_argument("--mode", type=str, default="eval", choices=["eval", "demo"], help="Evaluation-only fixed-speed baseline. No PPO model is loaded.")
    ...
    p.add_argument("--fixed-speed", type=float, default=1.35,
                   help="Fixed raw/commanded speed in m/s for the e2 fixed-speed WITHOUT governor baseline. PPO is not loaded.")
    p.add_argument("--baseline-name", type=str, default="e2_fixed_speed_no_governor",
...
class IndustrialE2EvalEnv(gym.Env):
    ...
    def __init__(self, args: argparse.Namespace, render_sim: bool = True):
        ...
        self.baseline_name = str(getattr(args, "baseline_name", "e2_fixed_speed_no_governor") or "e2_fixed_speed_no_governor")
        print(f"[BASELINE] {self.baseline_name}: fixed speed={self.fixed_speed_mps:.3f} m/s; governor OFF; PPO OFF")
    ...
    def step(self, action: np.ndarray):
        ...
        # The action is ignored; PPO is not loaded.  We still compute VSLAM risk,
        # corridor error, and yaw error for logging, but they do not change speed.
        ...
        gate, abort, reason = 1, 0, "no_governor_fixed_speed"
...
def evaluate(env: IndustrialE2EvalEnv, args: argparse.Namespace) -> None:
    """Run e2 fixed-speed/no-governor evaluation.

    This baseline intentionally does not load PPO and does not call model.predict.
    The environment ignores the action and uses --fixed-speed directly.
    """
    print(
        f"[E2_FIXED_EVAL] baseline={getattr(args, 'baseline_name', 'e2_fixed_speed_no_governor')} "
        f"fixed_speed={float(getattr(args, 'fixed_speed', 1.35)):.3f} m/s; PPO OFF; governor OFF"
    )
    ...
    print(f"[E2_FIXED_EVAL] metrics -> {env.summary_json}")
    print(f"[E2_FIXED_EVAL] paper row -> {env.result_table_csv}")
